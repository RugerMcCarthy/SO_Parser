...
public class ParseType64Utils {
    ...
    private static int SECTION_HEADER_SIZE = 64;
    ...
    public static void parseSectionHeaderList(byte[] fileByteArys) {
        ...
        long sectionHeaderOffset = Utils.byte2Long(elfContent.hdr.e_shoff);
    ...
    private static ElfType64.elf64_shdr getSectionHeader(byte[] header) {
        ...
        sectionHeader.sh_addr =  Utils.copyBytes(header, 16, 8);
        sectionHeader.sh_offset =  Utils.copyBytes(header, 24, 8);
        sectionHeader.sh_size =  Utils.copyBytes(header, 32, 8);
        sectionHeader.sh_link =  Utils.copyBytes(header, 40, 4);
        sectionHeader.sh_info =  Utils.copyBytes(header, 44, 4);
        sectionHeader.sh_addralign =  Utils.copyBytes(header, 48, 8);
        sectionHeader.sh_entsize =  Utils.copyBytes(header, 56, 8);
